...
class GeoHash
{
    ...
    function GetDistanceByPoint($lat1, $lng1, $lat2, $lng2)
    {
        ...
        $a = $radLat1 - $radLat2;
        $b = $radLng1 - $radLng2;
        $s = 2 * asin(sqrt(pow(sin($a / 2), 2) + cos($radLat1) * cos($radLat2) * pow(sin($b / 2), 2))) * 6371.393;
        return round($s, 1);
    }
     ...
     * @return float
     */
    function GetDistanceByHash($hash1, $hash2)
    {
        list($lat1, $lng1) = $this->decode($hash1);
        list($lat2, $lng2) = $this->decode($hash2);
        return $this->GetDistanceByPoint($lat1, $lng1, $lat2, $lng2);
    }
}
